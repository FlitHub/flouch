...
const getConfigFilename = (ext: string) => MPOUCH_CONFIG_BASE_NAME + ext;
...
const resolveConfigPathByTraversing = (
  ...
  const mPouchConfig = MPOUCH_CONFIG_EXT_ORDER.map((ext) =>
...
const makeResolutionErrorMessage = (
  ...
  "file, or a path to a directory. If directory is given, MPouchConfig will try to\n" +
  `traverse directory tree up, until it finds one of those files in exact order: ${MPOUCH_CONFIG_EXT_ORDER.map(
